...
- Fixed the unwanted Claude icon and menu entry from appearing in the NetBeans Tools menu
...
- Added **Alt+\** trigger action ("Trigger Inline AI Completion") in the editor actions list — assign a shortcut via Tools → Keyboard Shortcuts
- Added **Inline Completion** section in Tools → Options → Claude Code → General: enable/disable toggle, Auto/Manual trigger mode radio buttons, debounce delay spinner, and profile override combo box

- Fixed NetBeans .form files generated by Claude Code failing to open in Design view: the plugin now injects a system prompt into settings.local.json that instructs Claude Code to read the NetBeans form guide from MCP before creating any Swing UI
- Fixed JasperReports skill also injected into session system prompt so Claude Code reads it from MCP before generating .jrxml reports or PDF/Excel export code
- Fixed incorrect tool names in the NetBeans form guide (Section 22) — references to `mcp5_*` prefixed names are replaced with the actual MCP tool names that Claude Code uses
- Added Cursor CLI (`cursor-agent` / `agent`) as a selectable CLI type in Tools → Options → Claude Code → Advanced, with its own toolbar and session-tab icon
- Added a dedicated Google Antigravity toolbar and session-tab icon, which is now shown when the Antigravity CLI is selected (previously the Claude icon was used)
